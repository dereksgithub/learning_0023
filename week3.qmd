...
![Selecting the Landsat 8 and 9 Data with Overlap at Bristol for the second part of practical](images/Screenshot%202024-02-17%20173551.png){fig-align="center" width="600"}
...
![DN Results](images/wk3_01_band_2to4.png){fig-align="center" width="647"}
...
![QGis Clipping](images/Screenshot%202024-02-17%20181559.png){fig-align="center" width="461"}

![PCA Result Summary](images/wk3_pca_importance.png){fig-align="center"}

The first 3 components contain 86.6% of variances.

![PCA Result Plots](images/wk3_1st_pca_plot.png){fig-align="center" width="648"}

Using glcm to perform texture analysis for Landsat images. Homogeneity measures the closeness of the distribution of elements in the GLCM to the GLCM diagonal, indicating areas where pixel values are similar to their neighbors, while reflecting uniform textures within the image. Second moment, or energy, quantifies texture uniformity an indicator where higher values denote areas with more consistent or smooth textures. Correlation assesses how a pixel's value is predictably related to its neighbors, with high values indicating a linear or predictable relationship in gray-level values across the texture. For GLCM, Mean refers to the average intensity or gray level within the analyzed window, offering insights into the overall brightness or reflectance of the area being studied.

![GLCM 4 Methods](images/glcm.png){fig-align="center" width="648"}
...
-   **GNDVI (Green Normalized Difference Vegetation Index)** focuses on chlorophyll content, primarily on the green spectral region for enahnced sensitivity to vegetation density. [@gitelson1996use]
...
These indicators, are sufficient only for basic analysis demands, to derive more hidden relations, more modern techniques such as deep learning,[@ma2019deep] and Semi-supervised representation learning [@yan2020semi] can also be employed. Selecting proper metrics in remote sensing for an urban context is crucial in addressing specific planning or built-environment challenges. Among these indices, ARVI, NDWI, EVI, SAVI, MSAVI, NDMI and NDVI are still relevant in the industry and contemporary urban science. NDVI is widely used for monitoring urban vegetation health and urban green space. [@xue2017significant] NDWI plays a critical role in urban water body and drought monitoring, it is also important for urban fire prevention.[@zhu2017change] As for the technical part for this week's practical, coding part is smooth, however the data collection and alignment do require a high level of dedication to get right at first run.
