...
CART was originally proposed by Breiman et al. @breiman1984classification It is a type of predictive algorithm that can be used for both classification and regression. It splits data at each "node" with the goal of sub-setting the data into the target classes.

![Decision Tree used in the work by Winstral et al. [@winstral2002spatial] Pruned 16-node regression tree grown on Sx100 (°), D0 (dimensionless), elevation (m), net potential radiation index (W m–2), and slope. None of the splits were based on slope. Values within the ellipses and rectangles (terminal nodes) are the mean depth (m) of all samples falling within that node.](images/wk7/tree.png){fig-align="center" width="303"}
